...
saveImg <- function(name){
  if(save_img_sign){
    ggsave(paste("results", paste(name,".pdf"), sep = "/"), width = 10, height = 6, useDingbats=FALSE)
  }
}
...
file_path <- paste(file_study_folder, "max delta task", "", sep = "/")
...
for (file in filelist) {
  ...
  for(i in 1:6){
    data_group <- data_temp[data_temp$conditionId==(i-1),]
...
ciplot(data, "error", "conditionName", "titer-mean")
saveImg(paste(file_prefix, "error", "conditionName", "titer-mean",sep="-"))
...
df_stat_output <- stat_table_error(data,"C3-Palette Generation")
df_stat_output <- stat_table_error(data,"C3-Palette Assignment")
ciplot(data, "error", "conditionName", "error-mean")
saveImg(paste(file_prefix, "error", "conditionName", "error-mean",sep="-"))
