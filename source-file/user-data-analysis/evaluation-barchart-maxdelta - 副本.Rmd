...
for (file in filelist) {
  ...
  #data_temp <- data_temp[data_temp$conditionId!=3,]
...
```{r}
# Get the list of files.
file_path <- paste("counting class task", "", sep = "/")
filelist <- list.files(path = file_path, pattern = '*coSeparability.*?\\.csv$')

user_info <- read.csv(paste(file_path, "user_info-separability.csv", sep = ""), header = TRUE) 
...
for (file in filelist) {
  ...
  worker_id <- xx[[1]][1]
  print(worker_id)
  data_x <- user_info[user_info$workerId == as.character(worker_id),]
  # bind the data to the total data frame
  data_total <- rbind(data_total, data_x)
  
  
}
print(data_total)
write.csv(data_total, paste(file_path, "user_info-counting class task.csv", sep = ""))
```
