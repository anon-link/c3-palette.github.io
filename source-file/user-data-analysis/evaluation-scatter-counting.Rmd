...
#data_total <- data_total[data_total$conditionId!=3, ]
...
data_total <- data_total %>%
  mutate(conditionName = fct_relevel(conditionName, 
                                 "C3-Palette Generation","C3-Palette Assignment", 
                                 "Palettailor", 
...
data <- data_total
data$changeMagnitude <-factor(data$changeMagnitude)
model <- art(error ~ conditionName * changeMagnitude + (1|userName), data=data)
...
library(GFD)
set.seed(1234)
model <- GFD(error ~ conditionName * changeMagnitude, data = data, nperm = 1000, alpha = 0.05)
summary(model)
...
data_total <- data_total %>%
  mutate(count=1)
bar_data <- dplyr::summarise(group_by(data_total, error), frequency=sum(count))
p<-ggplot(bar_data, aes(x=error, y=frequency)) +
