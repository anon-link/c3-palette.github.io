...
FRNN.prototype.neighbors = function () {
	let pairs = this.search();
	// console.log(pairs);
	function toKey(pair) {
		return pair.id;
	...
	pairs.forEach(function (pair) {
		...
		if (!(key1 in neighbors)) {
			neighbors[key1] = [p2.id];
		} else {
			if (neighbors[key1].indexOf(p2.id) === -1)
				neighbors[key1].push(p2.id);
		}

		if (!(key2 in neighbors)) {
			neighbors[key2] = [p1.id];
		} else {
			if (neighbors[key2].indexOf(p1.id) === -1)
				neighbors[key2].push(p1.id);
		}
	});
