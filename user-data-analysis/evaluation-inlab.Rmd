...
library(forcats)
library(tidyverse)
file_study_folder <- "D:/论文/comparison/user study results/all_result_2/" #
```
...
for (file in filelist) {
  ...
  for(i in 1:nrow(group_data_total)){
    ...
    if(nrow(x1)==0){
      ...
      cat(toString(x["conditionId"][[1]]), " error rate is ", x1["count"][[1]]/x["count"][[1]], ", time is ",x["time"][[1]],"\n")
    }
  }
...
tttt <- data_total[data_total$changeMagnitude==0 & data_total$conditionId==0, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==0 & data_total$conditionId==1, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==0 & data_total$conditionId==2, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==0 & data_total$conditionId==3, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==1 & data_total$conditionId==0, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==1 & data_total$conditionId==1, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==1 & data_total$conditionId==2, ]
print(nrow(tttt))
tttt <- data_total[data_total$changeMagnitude==1 & data_total$conditionId==3, ]
print(nrow(tttt))
...
ciplot(data, "error", "condition", "Total")
ggsave("results/lab/total-error.pdf", width = 10, height = 6, useDingbats=FALSE)
ciplot(data, "totalTime", "condition", "Total")
ggsave("results/lab/total-time.pdf", width = 10, height = 6, useDingbats=FALSE)

ciplot(data[data$changeMagnitude==0, ], "error", "condition", "small Magnitude")
ggsave("results/lab/magnitude-error-small.pdf", width = 10, height = 6, useDingbats=FALSE)
ciplot(data[data$changeMagnitude==0, ], "totalTime", "condition", "small Magnitude")
ggsave("results/lab/magnitude-time-small.pdf", width = 10, height = 6, useDingbats=FALSE)

ciplot(data[data$changeMagnitude==1, ], "error", "condition", "medium Magnitude")
ggsave("results/lab/magnitude-error-medium.pdf", width = 10, height = 6, useDingbats=FALSE)
ciplot(data[data$changeMagnitude==1, ], "totalTime", "condition", "medium Magnitude")
ggsave("results/lab/magnitude-time-medium.pdf", width = 10, height = 6, useDingbats=FALSE)

ciplot(data[data$changeMagnitude==2, ], "error", "condition", "large Magnitude")
ggsave("results/lab/magnitude-error-large.pdf", width = 10, height = 6, useDingbats=FALSE)
ciplot(data[data$changeMagnitude==2, ], "totalTime", "condition", "large Magnitude")
ggsave("results/lab/magnitude-time-large.pdf", width = 10, height = 6, useDingbats=FALSE)
...
# scan length
```{r}
data_temp <- read.csv("D:/论文/comparison/user study results/all_data/user_scanlength.csv", header = TRUE)
names <- c(rep("Random Tableau 10", 24) , rep("Random Tableau 20", 24) , rep("Optimized Assignment", 24), rep("Ours Tableau 10", 24), rep("Ours Tableau 20", 24), rep("Ours Generation", 24))
value <- c( data_temp$Random.10, data_temp$Random.20,data_temp$Optimized.Tableau,data_temp$Ours.10,data_temp$Ours.20,data_temp$Ours.Generation)
...
data$names <- factor(data$names , levels=c("Random Tableau 10", "Random Tableau 20", "Optimized Assignment", "Ours Tableau 10","Ours Tableau 20","Ours Generation"))
...
data <- data %>%
  mutate(names = fct_relevel(names, 
                                 ...
                                 "Ours Tableau 20", "Ours Tableau 10", "Optimized Assignment", 
                                 "Random Tableau 10", "Random Tableau 20"))
ciplot(data, "value", "names", "Scan Length")
ggsave("results/scanlength.pdf", width = 10, height = 6, useDingbats=FALSE)
```
...
reference <- c("Ours Generation", "Ours Tableau 20", "Ours Tableau 10", "Optimized Assignment", "Random Tableau 10", "Random Tableau 20")
