...
# Generate the result table for accuracy in the discrimination tasks.
stat_table_accuracy <- function(data, compared_condition) {
  print(attr)
  columns <- c("Condition", "μ~95%CI", "W", "p-value", "d")
  conditions <- levels(data$conditionName) %>% rev
  print(conditions)
  ...
  for (index in 1:length(conditions)) {
    ...
    sub_data_one_group <- data %>% filter(conditionName==current_condition)
    # 95%CI
    df_stats[index, "μ~95%CI"] <- reportCI(sub_data_one_group, "accuracy")
    
    if(current_condition != compared_condition) {
      sub_data_two_groups <- data %>% filter(conditionName==compared_condition | conditionName==current_condition)
      # W test
      wt <- wilcox.test(accuracy ~ conditionName, 
                        data = sub_data_two_groups, 
                        conf.int=TRUE)
      df_stats[index, "W"] <- as.character(wt$statistic)
      df_stats[index, "p-value"] <- format(wt$p.value, format = "e", digits = 2)
      ...
      # Effect size
      df_stats[index, "d"] <- reportES(sub_data_two_groups, "accuracy", "conditionName")
    }
  }
  print(df_stats)
...
# Model: accuracy ~ ... datasetGroup + condition ...
model <- lm(accuracy ~ conditionName + datasetGroup + conditionName * datasetGroup, data)
anova(model)
summary(model)

# Model: accuracy ~ ... condition + magnitude ...
model <- lm(accuracy ~ conditionName + changeMagnitude + conditionName * changeMagnitude, data)
anova(model)
summary(model)

# Model: accuracy ~ ... condition + change type ...
model <- lm(accuracy ~ conditionName + changeType + conditionName * changeType, data)
anova(model)
summary(model)

# Model: for each change type: accuracy ~ ... condition + magnitude ...
model <- lm(accuracy ~ conditionName + changeMagnitude + conditionName * changeMagnitude, data[data$changeType=="pos", ])
anova(model)
summary(model)

# Model: for each change type: accuracy ~ ... condition + magnitude ...
model <- lm(accuracy ~ conditionName + changeMagnitude + conditionName * changeMagnitude, data[data$changeType=="num", ])
anova(model)
summary(model)
```
...
data <- data_total
file_prefix <- "cosaliency"#"coseparability"#
...
df_stat_output <- stat_table_accuracy(data,"C3-Palette Generation")
df_stat_output <- stat_table_time(data,"C3-Palette Generation")

df_stat_output <- stat_table_error(data,"C3-Palette Assignment")
df_stat_output <- stat_table_accuracy(data,"C3-Palette Assignment")
...
saveImg(paste(file_prefix,"accuracy", "conditionName", "Total",sep="-"))
...
df_stat_output <- stat_table_accuracy(data[data$changeType=="num", ],"C3-Palette Generation")
...
df_stat_output <- stat_table_accuracy(data[data$changeType=="num", ],"C3-Palette Assignment")
...
ciplot(data[data$changeType=="num", ], "accuracy", "conditionName", "num")
saveImg(paste(file_prefix,"accuracy", "conditionName", "num",sep="-"))
...
df_stat_output <- stat_table_accuracy(data[data$changeType=="pos", ],"C3-Palette Generation")
...
df_stat_output <- stat_table_accuracy(data[data$changeType=="pos", ],"C3-Palette Assignment")
...
ciplot(data[data$changeType=="pos", ], "accuracy", "conditionName", "pos")
saveImg(paste(file_prefix,"accuracy", "conditionName", "pos",sep="-"))
...
ciplot(data, "accuracy", "changeMagnitude", "Total")
saveImg(paste(file_prefix,"accuracy", "changeMagnitude", "Total",sep="-"))
ciplot(data, "totalTime", "changeMagnitude", "Total")
saveImg(paste(file_prefix,"totalTime", "changeMagnitude", "Total",sep="-"))
# ciplotManual(data[data$changeType=="pos", ], "error", "changeMagnitude", "pos")
# ciplotManual(data[data$changeType=="num", ], "error", "changeMagnitude", "num")
ciplot(data, "error", "changeType", "Total")
saveImg(paste(file_prefix, "error", "changeType", "Total",sep="-"))
ciplot(data, "accuracy", "changeType", "Total")
saveImg(paste(file_prefix, "accuracy", "changeType", "Total",sep="-"))
...
ciplotManual(data, "accuracy", "conditionMagnitude", "total")
saveImg(paste(file_prefix, "accuracy", "conditionMagnitude", "total",sep="-"))
...
ciplotManual(data[data$changeType=="num", ], "accuracy", "conditionMagnitude", "num")
saveImg(paste(file_prefix, "accuracy", "conditionMagnitude", "num",sep="-"))
...
ciplotManual(data[data$changeType=="num", ], "accuracy", "conditionMagnitude", "pos")
saveImg(paste(file_prefix, "accuracy", "conditionMagnitude", "pos",sep="-"))
...
#ggsave("counting_time_box.pdf", useDingbats=FALSE)

# 
# library(gridExtra)
# tt1 <- ttheme_minimal(base_colour = '#636363', base_size = 15, padding = unit(c(5, 10), "mm"))
# #pdf("counting_stats.pdf", height=4, width=6)
# grid.arrange(
...
abs(mean(df1$accuracy) - mean(df2$accuracy))
df <- df1 %>% rbind(df2)

powerAnalysisGraph(mean(df1$accuracy), mean(df2$accuracy), sd(df$accuracy))
saveImg(paste(file_prefix, "accuracy", "conditionName", "powerAnalysis.pdf",sep="-"))
```
### Time
```{r}
...
data <- data_total
for(i in 1:6){
  ciplotManual(data[data$datasetGroup == (i-1),], "accuracy", "conditionName", i)
}

# for different change magnitude, check error ~ conditionName
for(i in 1:3){
  ciplotManual(data[data$changeMagnitude == (i-1),], "accuracy", "conditionName", i)
}
...
# mean value
data <- dplyr::summarise(group_by(data_part, userName, conditionId), time=sum(titer))
#Statistical Test Results: mu~95% CI, W, p-value, d.
# error~conditionName and time~conditionName
df_stat_output <- stat_table_error(data,"C3-Palette Generation")
df_stat_output <- stat_table_error(data,"C3-Palette Assignment")
...
df_stat_output <- stat_table_error(data,"C3-Palette Generation")
df_stat_output <- stat_table_error(data,"C3-Palette Assignment")
ciplot(data, "titer", "conditionName", "Part-mean")
saveImg(paste(file_prefix,"titer", "conditionName", "Part-mean",sep="-"))
