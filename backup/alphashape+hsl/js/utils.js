...
function drawLineChart(data, svgId) {
    ...
    let svg_width = width - (svg_margin.left + svg_margin.right),
    svg_height = height - (svg_margin.top + svg_margin.bottom)
    // set the ranges
    var axis_x = d3.scaleLinear().range([0, svg_width]);
    var axis_y = d3.scaleLinear().range([svg_height, 0]);
    ...
    let linechart_svg = d3.select("#renderDiv").append("svg")
        ...
        .append("g")
        .attr("transform",
            "translate(" + (svg_margin.left + svg_margin.right) + "," + svg_margin.top + ")");
