...
let _findBestSuitableColor = function (c, origin, isHighlight) {
    let bg_contrast = d3_ciede2000(d3.lab(origin), d3.lab(bgcolor))
    // get new color through saturation
    let color = d3.hsl(c.h, (0 + c.s) / 2, c.l)
    let nd = getNameDifference(color, origin);
    let contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
    console.log(1, nd, contrast, bg_contrast, isHighlight);
    ...
    contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
    console.log(2, nd, contrast, bg_contrast, isHighlight);
    if (nd < 0.5 && (isHighlight && contrast >= bg_contrast || !isHighlight && contrast <= bg_contrast)) {
        return color;
    }
    ...
    nd = getNameDifference(color, origin);
    contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
    console.log(3, nd, contrast, bg_contrast, isHighlight);
    if (nd < 0.5 && (isHighlight && contrast >= bg_contrast || !isHighlight && contrast <= bg_contrast)) {
        return color;
    }
    ...
    nd = getNameDifference(color, origin);
    contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
    console.log(4, nd, contrast, bg_contrast, isHighlight);
    if (nd < 0.5 && (isHighlight && contrast >= bg_contrast || !isHighlight && contrast <= bg_contrast)) {
        return color;
    }
    ...
    contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
    let count = 0;
    while (getNameDifference(color, origin) > 0.5 || (isHighlight && contrast < bg_contrast || !isHighlight && contrast > bg_contrast)) {
        color = d3.hsl(c.h, _disturbValue(c.s), _disturbValue(c.l));
        contrast = d3_ciede2000(d3.lab(color), d3.lab(bgcolor))
        count++
        if (count > 100) {
    ...
    let _optimizeSL = function () {
        ...
        while (cur_temper > end_temper) {
            for (let i = 0; i < 1; i++) {//disturb at each temperature
                ...
                bg_contrast_array[disturb_index] = d3_ciede2000(d3.lab(color_palette_cur[disturb_index]), d3.lab(bgcolor));
        ...
        lineChart_data.sort((a, b) => (b.score - a.score))
        for (let j = 0; j < 5; j++) {
            for (let i = 0; i < source_datasets.length; i++) {
                let used_palette = lineChart_data[j].palette;
